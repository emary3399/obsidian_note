...
> [!CHECKLIST] 执行前必查
> - [x] 备份数据 (`CREATE TABLE backup AS SELECT * FROM 原表`)
> - [x] 检查列当前定义  
>   ```sql
>   DESC 表名;  -- MySQL
>   \d+ 表名    -- PostgreSQL
...
> | 场景                  | 是否允许       | 原因                 |
> |-----------------------|--------------|---------------------|
> | 同一语句内后定义别名     | ✅ 允许       | SQL标准明确规定       |
> | 跨独立语句引用别名       | ❌ 禁止       | 作用域隔离            |
> | 嵌套子查询层级引用       | ⚠️ 有条件允许 | 需每层显式定义        |

> [!EXAMPLE] ​**各数据库实测**​
> ```sql
> -- MySQL/PostgreSQL/SQLite均支持
> SELECT t.title FROM books AS t;
> 
> -- SQL Server/Oracle同样合法
> SELECT e.salary FROM hr.employees AS e;
> ```

> 💡 ​**最佳实践建议**​
> 1. 保持别名命名语义化（如`s1`表学生，`sc`表成绩）
> 2. 复杂查询时显式定义所有别名
> 3. 避免在工具链中使用极端写法
> 
> > 📚 原理总结：  
> > SQL采用**声明式语法**，只要逻辑正确，书写顺序不影响执行
...
> [!NOTE] ​**ON DUPLICATE KEY UPDATE 执行分析**​
> 
> ### 🔍 语句功能解析
> ```sql
...
> 
> ### ✅ 执行结果解读
> `Query OK, 2 rows affected` 表示：
> 1. ​**尝试插入**新记录（计为1行影响）
> 2. ​**发现主键/唯一键冲突**后转为执行UPDATE（再计为1行影响）
> 3. 最终更新了已有记录的`second_column`字段
